...
import { getClassPreList } from '@/apis/class'
...
const PreviewList = () =>{
    ...
    const [isLoading,setIsLoading] = useState(false)
     ...
     const getPreList = async() =>{
       setIsLoading(true)
       let {data} = await getClassPreList(c_id)
       ...
       {isLoading ? <LoadingComponent /> : <>
            <div style={{width:'100%',height:'100%',display:'flex'}}>
               ...
               <div style={{width:'80%',height:'100%'}}>
                <div style={{fontFamily:'siyuan',fontSize:16,paddingBlockStart:30,textAlign:'left',marginBottom:10}}><span style={{marginRight:10,color:'#0B2273',fontSize:20,cursor:'pointer'}} onClick={()=>navigate(`/class/${c_id}`)}><ArrowLeftOutlined /></span>预习任务</div>
                ...
                {getCurrentPageData()?.map((item,index)=>(
                          ...
                          <div style={{textAlign:'left',fontSize:21,fontWeight:'bold',lineHeight:1,fontFamily:'siyuan',height:'max-content'}}>{item.active === 1 ? <span className={style.ingTag}>进行中</span> : <span className={style.endTag}>已结束</span>}{item.previewName}<span style={{fontSize:14}}>（ {item.ptitle}--{item.secondaryTitle} ）</span></div>
                          <div style={{flex:1}}></div>
                          <div style={{lineHeight:1,display:'flex',color:'#757575',fontSize:12,textAlign:'left'}}>
                              <span >截止时间: {item.deadline}</span>
                              <span style={{flex:1}}></span>
                              <span style={{marginRight:20}}>已提交： {item.complete}</span>
                              <span >未提交： {item.unComplete}</span>
                              </div>
                          </div>
                          <div style={{flex:0.5,height:80,display:'flex',justifyContent:'center',alignItems:'center'}}>
                          <button  className={style.functionBtn} onClick={()=>navigate(`/preview/${item.ptId}`)}>查看作业</button>
                          <button  className={style.functionBtn} onClick={()=>navigate(`/class/${c_id}/detail/${item.ptId}`)}>答题详情</button>
                          </div>
