...
public class API {
    public static final String BASE_URL = "https://open.docmee.cn";
...
//        "length": "medium", // 篇幅长度：short
        body.put("length", "short");
        body.put("prompt", "");//不做要求
        HttpUtils.HttpRequest httpRequest = HttpUtils.HttpRequest.postJson(url);
        httpRequest.setBody(body.toJSONString());
        httpRequest.addHeaders("token", apiToken);
    ...
    public static String generateContent(String apiToken, String outlineMarkdown, String dataUrl, String prompt) {
        String url = BASE_URL + "/api/ppt/generateContent";//https://open.docmee.cn/api/ppt/v2/generateContent
        JSONObject body = new JSONObject();
...
//    TODO 获取模版列表，然后赛选出指定的模版，返回id
    public static String selectOneTemplateId(String apiToken){
        String styleValue = getStyleValue(apiToken, "商务科技");
        // 2. 使用获取的风格值筛选模板
        String url = BASE_URL + "/api/ppt/randomTemplates";
        JSONObject body = new JSONObject();
        body.put("size", 10);  // 获取10个模板增加找到匹配项的几率

        // 构建过滤条件
        JSONObject filters = new JSONObject();
        filters.put("type", 1);  // 系统模板
        filters.put("style", styleValue);  // 设置风格过滤值
        body.put("filters", filters);
        ...
        // 在返回结果中查找匹配的模板
        JSONArray templates = result.getJSONArray("data");
        for (int i = 0; i < templates.size(); i++) {
            JSONObject template = templates.getJSONObject(i);
            if (styleValue.equals(template.getString("style"))) {
                return template.getString("id");
            }
        }

        throw new RuntimeException("未找到匹配'商务科技'风格的模板");
    }

//    TODO 辅助方法
private static String getStyleValue(String apiToken, String styleName) {
    String url = BASE_URL + "/api/ppt/template/options";
    ...
    // 处理响应
    if (response.getStatus() != 200) {
        throw new RuntimeException("获取选项失败，httpStatus=" + response.getStatus());
    }

    JSONObject result = response.getResponseToJson();
    if (result.getIntValue("code") != 0) {
        throw new RuntimeException("获取选项异常，" + result.getString("message"));
    }
    ...
    for (int i = 0; i < styles.size(); i++) {
        ...
        if (styleName.equals(style.getString("name"))) {
            return style.getString("value");
        }
    }

    throw new RuntimeException("未找到风格名称: " + styleName);
}
